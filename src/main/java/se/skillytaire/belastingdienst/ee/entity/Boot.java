...
public class Boot extends AbstractEntity<Boot, Integer> {
   ...
   public Boot(Verhuurder eenEigenaar, final PositiveInteger nummer) {
      if(eenEigenaar == null) throw new IllegalArgumentException("eenEigenaar mag niet 'null' zijn");
      if(nummer == null) throw new IllegalArgumentException("Nummertje!");
   ...
   public Boot(final Boot that) {
      ...
      this.tochtGeschiedenis = (ArrayList<Tocht>) that.tochtGeschiedenis.clone();
   ...
   private boolean isVrij() {
      return ((!this.hasLaatsteTocht())
            || (this.deLaatsteTocht.isBeeindigd()));
   }
