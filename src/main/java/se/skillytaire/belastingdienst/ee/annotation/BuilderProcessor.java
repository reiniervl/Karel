   ...
   public boolean process(final Set<? extends TypeElement> annotations,
      ...
      for (TypeElement annotation : annotations) {
         for (Element element : roundEnv.getElementsAnnotatedWith(annotation)) {
            ...
            this.processFields(element, enclosedElements);
         }
      }
      return false;
   }

   private void processFields(final Element enclosing,
         final List<Element> fields) {
      try {
         String packageName = this.processingEnv.getElementUtils()
               .getPackageOf(enclosing).toString();
         String className = enclosing.getSimpleName() + "Builder";
         JavaFileObject srcFile = this.processingEnv.getFiler()
               .createSourceFile(packageName + "." + className);
         try (Writer writer = srcFile.openWriter()) {
            writer.write("package " + packageName + ";");
            writer.write("\n\npublic class " + enclosing.getSimpleName()
                  + "Builder {");
            writer.write(String.format("\n\tprivate %s build = new %s();\n",
                  enclosing.getSimpleName(), enclosing.getSimpleName()));

            for (Element field : fields) {
               if (field.getAnnotation(BuilderField.class) == null) {
                  continue;
               }
               writer.write(String.format("\n\tpublic %s set%s(%s %s) {",
                     className,
                     field.getSimpleName().toString().substring(0, 1)
                           .toUpperCase()
               ...
               // field.getSimpleName(), field.getKind(),
