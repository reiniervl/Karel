...
public class VerhuurderJpaDAOTest {
	...
	public void testDaoAdd() {
		...
		Assert.assertTrue("Verhuurder is opgeslagen", verhuurder.isPersistant());
	...
	public void testNewVerhuurder() {
		Verhuurder verhuurder = generateRandomHuurder();
		this.beanUnderTest.add(verhuurder);
		Assert.assertTrue(verhuurder.isPersistant());
	}
	...
	// 	if(!thisVerhuurder.isPersistant()) {
	// 		this.addWithTX(this.thisVerhuurder);
	// 	}
	// 	Optional<Verhuurder> result = this.beanUnderTest.findByOID(this.thisVerhuurder.getOid());
